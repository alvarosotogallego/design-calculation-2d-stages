...
global gastoM presion0 temperatura0 omegaMatriz espesorMatriz omega SM
...
[SM_NEW_i,VM_NEW_i] = radial(i,N,M,mass_frac,SM,Vm,gamma,R,T,delta_z,r);

SM_NEW(1:N,i) = SM_NEW_i(1:N,i);
Vm_NEW(1:N,i) = VM_NEW_i(1:N,i);

end

SM_NEW(1:N,1) = SM(1:N,1);
clear r_NEW_i VM_NEW_i RFr_i

%% Converge-Radial Change
[R_flag,Vm,r,z,r_iter] = converge2(SM,SM_NEW,M,N,Vm_NEW,Vm,r_iter,z,r);
Vz = Vm.* cos(phi);
V_tot = sqrt( Vm.^ 2 + V_t.^ 2 );
