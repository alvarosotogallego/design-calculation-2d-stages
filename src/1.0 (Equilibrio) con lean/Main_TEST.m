...
global epsilon psi rel_f i_alabe2 i_alabe
global gastoM presion0 temperatura0 omegaMatriz
global PerdidaCT PerdidaCH PerdidaOMEGA Theta Dflujo
...
num(j,i) = r(j,i) * Vm(j,i) * tan(Dflujo(j,i)) - r(j,i-1) * Vm(j,i-1) * tan(Dflujo(j,i-1));
    end
    
delta_m(j,i) = sqrt( ((r(j,i) - r(j,i-1))) ^ 2 + (z(i) - z(i-1)) ^ 2);
...
for i = 1:M
for j = 1:N
FN(j,i) = Vm(j,i) / r(j,i) * (tan(Dflujo(j,i)) * tan(phi(j,i) + psi(j,i)) +    1 *(tan(epsilon(j,i)) / cos(phi(j,i) + psi(j,i)))   )*d1dm(j,i); %%%%%%%%%
FM(j,i) = Vm(j,i) * tan(Dflujo(j,i)) / r(j,i) * d1dm(j,i);
end
end
...
Pinicial = (Pin/N)/100000

Pfinal = (Pend/N)/100000

RelCompresion = (Pend/N)/(Pin/N)

Potencia = gastoM*Cp*(T0in-T0end)

PhiCuvas = zeros([50 100]);
...
% plot(z',r','k');
% subplot(3,2,3);
% pcolor(z,r,T); shading interp;
% colorbar;
% title('T');
% hold on
% plot(z',r','k');
% subplot(3,2,4);
% pcolor(z,r,T_0); shading interp;
% colorbar;
% title('T_0');
% hold on
% plot(z',r','k');
% subplot(3,2,5);
% pcolor(z,r,Theta*180/3.1416);
% colorbar;
% title('Theta');
% hold on
% plot(z',r','k');
