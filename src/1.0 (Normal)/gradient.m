...
p_0,V_tot,p,delta_m,rV_t2,M,omega_m,ct,ch)
for j = SL_Mid:N
omega_r(j,i) = omega_m * ct * ( 2 * (r(j,i) - r(1,i) ) / ...
(r(N,i) - r(1,i)) - 1) ^ 3;
end
for j = 1:SL_Mid-1;
omega_r(j,i) = omega_m * ch * ( 1 - 2 * (r(j,i) - r(1,i)) / ...
(r(N,i) - r(1,i))) ^ 3;
end
for j = 1:N
...
for j = 1:N
if i == 1
delta_s(j,i) = Cp * ((gamma / (gamma-1)) * log((p_0(j,i) - ...
abs(delta_po(j,i))) / p_0(j,i)) - log(T_0(j,i) / ...
T_0(j,i)));
else
delta_s(j,i) = Cp * ((gamma / (gamma-1)) * log((p_0(j,i-1) - ...
abs(delta_po(j,i))) / p_0(j,i-1)) - log(T_0(j,i) / ...
T_0(j,i-1)));
end
end
for j=1:N
