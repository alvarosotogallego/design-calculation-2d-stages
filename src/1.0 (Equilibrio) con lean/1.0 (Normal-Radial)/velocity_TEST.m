...
FM,FN,T_0,V_t,bladeTest,rel_f,omegaMatriz,Cp,gamma,T,p,p_0,s,H,H_0,R,V_tot,V_tREL1,V_tREL,V_ang,TIPO,z)
...
for j = 1:N
A(j) = 0.4 * 2 * (cos(phi(j,i) + psi(j,i)) * C(j,i) + sin(phi(j,i) + psi(j,i)) / Vm(j,i) * dvmdm(j,i));
if TIPO == 2
B(j) = 0.3 * 2 * (dHodq(j,i) - T_0(j,i) * dsdq(j,i) - 1 / (2 * r(j,i) ^ 2) * drvtdr(j,i) - sin(phi(j,i) + psi(j,i)) * FM(j,i) - cos(phi(j,i) + psi(j,i)) * FN(j,i));
% B(j) = 0.3 * 2 * (dHodq(j,i) - T_0(j,i) * dsdq(j,i) - 1 / (2 * r(j,i) ^ 2) * drvtdr(j,i));
else
B(j) = 0.4 * 2 * (dHodq(j,i) - T_0(j,i) * dsdq(j,i) - 1 / (2 * r(j,i) ^ 2) * drvtdr(j,i) - sin(phi(j,i) + psi(j,i)) * FM(j,i) - cos(phi(j,i) + psi(j,i)) * FN(j,i));
% B(j) = 0.5 * 2 * (dHodq(j,i) - T_0(j,i) * dsdq(j,i) - 1 / (2 * r(j,i) ^ 2) * drvtdr(j,i));
end
end

for j = SL_Mid+1:N
% A_Bar(j) = (A(j-1) + 9*A(j)) / 10;
% B(j) = (B(j-1) + B(j)) / 2;
A_Bar(j) = (A(j-1) + A(j)) / 2;
VM2NEW(j) = exp( -A_Bar(j) * (r(j,i) - r(j-1,i)) ) * (VM2NEW(j-1) + ((r(j,i) - r(j-1,i)) * B(j-1)) / 2 ) + B(j) * (r(j,i) - r(j-1,i)) / 2;
end
for j = SL_Mid-1:-1:1
% A_Bar(j) = (A(j+1) + 9*A(j)) / 10;
% B(j) = (B(j+1) + B(j)) / 2;
A_Bar(j) = (A(j+1) + A(j)) / 2;
VM2NEW(j) = exp( A_Bar(j) * (r(j+1,i) - r(j,i)) ) * (VM2NEW(j+1) + ((r(j+1,i) - r(j,i)) * -B(j+1)) / 2) - B(j) * (r(j+1,i) - r(j,i)) / 2;
...
for j=1:N    
    ...
    p_0(j,i) = real(p_0(j,i));

    s(j,i) = Cp * log(T_0(j,i)) - R * log(p_0(j,i));
...
for j = 2:N
Tube_Flow(j) = (r(j,i)^2 - r(j-1,i)^2 + r(1,i)*2*(z(j,i)-z(j-1,i)) )* rho(j,i) * Vm(j,i) * pi; 
...
Vm_MID = min([ 1.05 * Vm_MID (2 - Mass_Flow / mdot_tot) * Vm_MID ]);
elseif Mass_Flow / mdot_tot > 1
Vm_MID = max([ 0.95 * Vm_MID (2 - Mass_Flow / mdot_tot) * Vm_MID ]);
end
