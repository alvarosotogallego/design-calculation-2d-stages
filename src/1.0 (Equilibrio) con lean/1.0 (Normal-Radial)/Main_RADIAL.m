...
% for j=1:N
%             
%     if bladeTest(j,i) == 0 & bladeTest(j,i+1) == 2 & j == 2
% 
%         PB12(2) = -180/3.1416*atan(V_t(j,i)/Vm(j,i))
% 
%     elseif bladeTest(j,i) == 0 & bladeTest(j,i+1) == 2 & j == N
% 
%         PB12P(2) = -180/3.1416*atan(V_t(j,i)/Vm(j,i))
%         
%     end
% end
% end
...
mdot_perR2(j,i) = (r(j,i) ^ 2 - r(j - 1,i) ^ 2 + +  r(1,i)*2*(z(j,i)-z(1,i)) )*( (rho(j,i)+rho(j-1,i))/2 * (Vm(j,i)+Vm(j-1,i))/2 * pi);
...
Vm(j,i) = mdot_perR2(j,i) / (rho(j,i) * pi *((r(j,i))^2 - (r(j-1,i))^2 - (espesorMatriz(j,i))^2 +  r(1,i)*2*(z(j,i)-z(1,i))));
